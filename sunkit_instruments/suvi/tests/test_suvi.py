...
def test_suvi_testdata_exists(tmp_path):
    ...
    dst2 = os.path.join(
        rootdir, "dr_suvi-l2-ci171_g16_s20211231T000800Z_e20211231T001200Z_v1-0-1.fits"
    )
    dst3 = os.path.join(
        rootdir,
    ...
    if not os.path.exists(dst3):
        try:
            f3 = requests.get(
                "https://noaa-goes16.s3.amazonaws.com/SUVI-L1b-Fe171/2021/365/00/"
                + "OR_SUVI-L1b-Fe171_G16_s20213650022109_e20213650022119_c20213650022323.nc"
            )
            open(
                os.path.join(
...
def test_suvi_despiking():
    ...
    L1b_fits_header, L1b_fits_data, L1b_fits_dqf = suvi.read_suvi(
        L1b_fits, return_DQF=True
    )
